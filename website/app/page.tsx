...
const copy = {
  zh: {
    ...
    featuresTitle: "能力更完整，\n操作依然简单",
    features: [
      ["灵活截图", "区域、窗口、屏幕、滚动长图、3 秒延迟，以及重复上次区域"],
      ["OCR 插件与本机脱敏", "OCR 默认在本机运行，也可为主动框选区域配置远程服务；历史索引与脱敏始终留在本机"],
      ["选区微调", "八个控制点、精确宽高、比例锁定与方向键像素级调整"],
      ["清晰度控制", "支持原生 Retina、720p、1080p、2K、4K 与 8K 输出"],
      ["直接贴屏", "截图后立即固定在桌面，并可同时保留任意多张贴图"],
      ["贴图控制", "等比缩放、透明度、鼠标穿透、阴影和批量显隐"],
      ["可搜索历史", "数量与期限可配置，本机 OCR 搜索、逐项管理并创建截图拼板"],
      ["自动化入口", "全局快捷键、URL Scheme 与 macOS 快捷指令串联常用流程"],
      ["快速结果", "截图后立即复制、保存、标注、贴屏，或拖入任意 App"],
    ],
    ...
    comparisonCards: [
      {
        ...
        difference: "PinboardShot 不把截图工具做成上传中枢；远程 OCR 只有在用户配置插件并主动框选区域后才会运行。",
      },
      {
        name: "Mac-native",
        label: "贴近 macOS 行为",
    ...
    comparisonSummary: ["Mac 原生", "应用无账号", "默认本机处理", "远程 OCR 需主动配置"],
    privacyEyebrow: "PRIVATE BY DESIGN",
    privacyTitle: "默认本机处理，\n远程 OCR 由你决定",
    privacyBody: "Mac 应用不需要账号，没有云同步，也没有遥测。截图、标注、历史和偏好设置默认在设备本地处理；只有你明确配置远程 OCR 插件并主动框选 OCR 区域时，该区域才会发往指定服务，历史索引与智能脱敏不会上传",
    privacyPoints: ["远程 OCR 默认关闭", "API Key 存入钥匙串", "历史与脱敏保持本机", "无遥测"],
  ...
  en: {
    ...
    features: [
      ["Flexible capture", "Capture an area, window, display, scrolling page, delayed shot, or the last region again."],
      ["OCR plugins & local redaction", "Use on-device OCR by default or configure a remote service for actively selected regions; history indexing and redaction stay local."],
      ["Precise selection", "Use eight handles, exact dimensions, aspect lock, or pixel-level arrow-key adjustments."],
      ...
      ["Searchable history", "Choose limits and retention, search local OCR text, manage items, and build comparison boards."],
      ["Automation", "Connect common workflows through global shortcuts, a URL scheme, and macOS Shortcuts actions."],
      ["Quick result", "Copy, save, annotate, pin, or drag a finished capture into any app."],
    ],
    ...
    comparisonCards: [
      {
        ...
        strength: "Captures, annotations, recent history, and preferences are handled locally by default without requiring an account or cloud sync.",
        difference: "PinboardShot is not an upload hub. Remote OCR stays disabled until the user configures a plugin and actively selects a region.",
      },
      {
        name: "Mac-native",
    ...
    comparisonSummary: ["Native Mac", "No account in the app", "Local by default", "Remote OCR is opt-in"],
    privacyEyebrow: "PRIVATE BY DESIGN",
    privacyTitle: "Local by default.\nRemote OCR is your choice.",
    privacyBody: "The Mac app has no account, cloud sync, or telemetry. Captures, annotations, history, and preferences stay on-device by default. A region is sent only when you configure a remote OCR plugin and actively select it; history indexing and smart redaction are never uploaded.",
    privacyPoints: ["Remote OCR off by default", "API Keys in Keychain", "Local history and redaction", "No telemetry"],
    trustEyebrow: "TRUST SIGNALS",
